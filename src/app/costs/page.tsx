...
function Money({ t, className = "" }: { t: CostTotals; className?: string }) {
  ...
  return (
    <span className={className}>
      ...
      {short > 0 && (
        <span
          className="ml-1 text-[11px] font-normal text-amber-400"
    ...
    <div className="mx-auto max-w-5xl">
      <header className="mb-5 flex flex-wrap items-start justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          <StoreIcon size={16} className="text-zinc-500" />
          <select
      ...
      <div className="mb-5 grid grid-cols-2 gap-3 md:grid-cols-4">
        ...
        <Stat
          label="Priced foods"
          value={
            <span className={coverage.priced === coverage.total ? "text-emerald-400" : ""}>
              {coverage.priced}
              <span className="text-zinc-500">/{coverage.total}</span>
            </span>
          }
          hint={`At ${storeName}`}
        />
      ...
      {selectedStoreId !== BASE_STORE_ID && (
        <div className="mb-5 flex flex-wrap items-center justify-between gap-3 rounded-2xl card px-4 py-3 text-sm">
          <p className="text-zinc-400">
            Blank prices at <span className="text-zinc-200">{storeName}</span> fall back to your base
              ...
              <div className="overflow-hidden rounded-2xl card">
                {byCategory.get(c.key)!.map((f) => {
                  ...
                  return (
                    <div
                      key={f.id}
                      className="flex items-center gap-3 border-b border-white/[0.06] px-4 py-2.5 last:border-0"
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="text-[11px] text-zinc-500">
                          ...
                          {effective && effective.source === "base" && selectedStoreId !== BASE_STORE_ID && (
                            <span className="ml-2 text-sky-400/80">using base price</span>
                          )}
                          {!effective && (
                            <span className="ml-2 text-amber-400/90">no price yet</span>
                          )}
                        </div>
                      </div>
                      <div className="flex shrink-0 items-center gap-1.5">
                        ...
                        />
                        <span className="w-9 text-left text-[11px] text-zinc-500 sm:w-12 sm:text-xs">
                          /{unitLabel(f.unit)}
                        </span>
                        <button
                          ...
                          className="hidden rounded-lg p-1.5 text-zinc-600 enabled:hover:bg-white/[0.06] enabled:hover:text-zinc-300 disabled:opacity-30 sm:block"
                        >
                          <RotateCcw size={14} />
                        </button>
                      </div>
      ...
      <section className="mt-8">
        <div className="mb-2 flex items-center justify-between">
          ...
          <span className="text-[11px] text-zinc-500">at {storeName}</span>
        </div>
        <div className="overflow-hidden rounded-2xl card">
          <div className="flex items-center gap-3 border-b border-white/[0.06] bg-white/[0.02] px-4 py-2 text-[11px] font-medium uppercase tracking-wide text-zinc-500">
          ...
          {recipeRows.map(({ recipe, total, per }) => (
            <div key={recipe.id} className="flex items-center gap-3 border-b border-white/[0.06] px-4 py-2.5 text-sm last:border-0">
              <span className="min-w-0 flex-1 truncate">
                {recipe.emoji} {recipe.name}
                <span className="ml-2 text-[11px] text-zinc-500">{recipe.servings} servings</span>
              </span>
              <span className="w-28 text-right tabular-nums">
                {isComplete(per) || per.priced > 0 ? fmtMoney(per.cost) : <span className="text-zinc-600">—</span>}
              </span>
        ...
        {recipeRows.some((r) => !isComplete(r.total)) && (
          <p className="mt-2 flex items-start gap-2 text-[11px] text-amber-400/90">
      ...
      <p className="mt-6 text-center text-xs text-zinc-500">
        ...
        <Link href="/stores" className="text-emerald-400 hover:underline">
          Pick a store
        </Link>{" "}
